...
# --- Song Class ---
class Song:
    def __init__(self, title, artist, file_data=None):
        self.title = title
        self.artist = artist
...
class MusicPlaylist:
    ...
    def add_song(self, title, artist, file_data=None):
    ...
    def play_current_song(self):
        if self.current_song:
            ...
            if self.current_song.file_data:
                st.audio(self.current_song.file_data, format='audio/mp3')
            else:
                st.caption("(No audio file uploaded for this song)")
...
uploaded_file = st.sidebar.file_uploader("Upload Audio File", type=["mp3"])
